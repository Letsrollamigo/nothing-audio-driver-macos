...
public enum NothingCatalog {
    ...
    /// Чем модель управляет звучанием: пресетами эквалайзера, профилями
    /// звучания или ничем. Полосой прошивки не управляется — набор кнопок
    /// задан разметкой страницы, а страница у модели одна.
    ///
    /// Запасной ответ на неизвестную модель — полный набор пресетов, тот
    /// самый, что экран показывал до появления этого различия. Промах здесь
    /// возможен только при промахе опознания, и потерять контрол целиком
    /// было бы хуже, чем показать лишнюю кнопку.
    public static func sound(model id: String) -> SoundStyle {
        models.first { $0.id == id }?.sound

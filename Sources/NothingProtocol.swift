...
public enum NothingProtocol {
    ...
    /// в раскладке чтения нет вовсе.
    ///
    /// Проверено на B170 03.09.2026: устройство принимает обе формы — и с
    /// хвостом, и без него, — а читает обратно одинаково, без хвоста. То есть
    /// хвост не обязателен и не хранится; форматом он не является. Оставлен
    /// сознательно: у нас одна модель, донор шлёт его всем, и лишние
    /// двадцать четыре нуля дешевле, чем догадка про чужую прошивку.
